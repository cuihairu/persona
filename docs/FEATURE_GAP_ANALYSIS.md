...
- Vaults/Items
  ...
  - Attachments/versioning: [≈] blob store ready (attachments UI pending); item change history recorded on every create/update/delete with metadata-only field diffs and surfaced in desktop detail pane + `persona credential history` (2026-09; restore-to-version pending)
- Autofill & Browser

...
- **设备移除的诚实边界**：被移除设备已知道 group key，**已同步历史的密钥不可
  ...
  各设备重新拉取），作为远期功能登记（§11）；第一阶段只做「吊销该设备的
  认证与访问」并如实告知边界。
...
- **吊销**：设备管理页/CLI 删除该设备：服务器删其公钥与信封、吊销其 token。
  前向边界见 DR-3（诚实提示文案照 THREAT_MODEL 风格）。
...
第一阶段实现时在 `THREAT_MODEL.md` 新增「E2EE 同步」章，骨架如下（阶段 0 定稿，
正文随阶段 1–2 写实）：
...
2. **安全轮换**（group key rotate + 全量重包）的触发 UX 与中断恢复——设备移除
   的真正闭环依赖它。
3. **oplog 增长与墓碑保留策略**：无限追加不可持续；保留窗口/TTL 与 TODO L358
   既有的「保留策略/TTL」follow-up 合并处理。

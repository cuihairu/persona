...
use persona_core::{
    Database,
    models::{Workspace, AuditLog, AuditAction, ResourceType},
    storage::{WorkspaceRepository, AuditLogRepository},
};
...
pub async fn execute(args: MigrateArgs, config: &crate::config::CliConfig) -> Result<()> {
    ...
    match repo.find_by_path(&path_str).await? {
        Some(mut ws) => {
    ...
    println!("  Created new workspace row: {}", if created { "yes".green() } else { "no".dim() });
    println!();

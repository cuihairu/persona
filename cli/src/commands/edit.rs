...
fn show_current_values(identity: &Identity) -> Result<()> {
    ...
    println!("  {}: {}", "Name".dim(), identity.name.cyan());
    ...
    if !identity.tags.is_empty() {
        println!("  {}: {}", "Tags".dim(), identity.tags.join(", ").dim());
    }

    if !identity.attributes.is_empty() {
        println!("  {}: {} custom attributes", "Attributes".dim(), identity.attributes.len());
    }
    ...
    match field.to_lowercase().as_str() {
        ...
        "email" => {
            if let Some(new_email) = value {
                identity.email = if new_email.is_empty() { None } else { Some(new_email) };
            } else {
                edit_email(identity)?;
            }
        }
        "phone" => {
            if let Some(new_phone) = value {
                identity.phone = if new_phone.is_empty() { None } else { Some(new_phone) };
            } else {
                edit_phone(identity)?;
            }
        }
...
fn edit_from_args(identity: &mut Identity, args: &EditArgs) -> Result<()> {
    ...
    if let Some(ref new_email) = args.email {
        identity.email = if new_email.is_empty() { None } else { Some(new_email.clone()) };
    }

    if let Some(ref new_phone) = args.phone {
        identity.phone = if new_phone.is_empty() { None } else { Some(new_phone.clone()) };
    }
...
fn edit_identity_type(identity: &mut Identity) -> Result<()> {
    ...
    let current_index = types.iter().position(|&t| t == identity.identity_type).unwrap_or(0);
    ...
    println!("{} Type updated to: {}", "✓".green(), identity.identity_type.cyan());
...
fn add_custom_attribute(identity: &mut Identity) -> Result<()> {
    ...
    identity.attributes.insert(key.clone(), Value::String(value));
...
fn edit_existing_attribute(identity: &mut Identity) -> Result<()> {
    ...
    let current_value = identity.attributes.get(key)
        .and_then(|v| v.as_str())
        .unwrap_or("");
    ...
    identity.attributes.insert(key.clone(), Value::String(new_value));
...
fn show_changes_summary(identity: &Identity) -> Result<()> {
    ...
    println!("  Modified timestamp: {}", identity.modified.dim());

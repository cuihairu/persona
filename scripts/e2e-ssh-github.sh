...
existing=$(persona ssh list-all | grep -B1 "Name: $KEY_NAME" | grep -oP 'ID: \K[0-9a-f-]+' || true)
if [ -n "$existing" ]; then
  key_id=$existing
  echo "✓ 复用既有密钥 $key_id"
else
  persona ssh generate --identity "$IDENTITY" --name "$KEY_NAME"
  key_id=$(persona ssh list-all | grep -B1 "Name: $KEY_NAME" | grep -oP 'ID: \K[0-9a-f-]+' || true)
...
persona ssh agent-status | grep -qi running || fail "agent 未进入 running 状态"
echo "✓ agent running"

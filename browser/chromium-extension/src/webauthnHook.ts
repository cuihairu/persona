...
(() => {
    ...
    if (win.__personaWebauthnHook) return;
    ...
    function toBytes(source: BufferSource | undefined): Uint8Array | undefined {
        if (source === undefined || source === null) return undefined;
        if (source instanceof ArrayBuffer) return new Uint8Array(source);
        return new Uint8Array((source as ArrayBufferView).buffer,
    ...
    function creationOptionsToJson(options: PublicKeyCredentialCreationOptions): Record<string, any> {
     ...
     */
    function clientDataJson(type: 'webauthn.create' | 'webauthn.get', challenge: Uint8Array): Uint8Array {
    ...
    function shouldIntercept(options: CredentialRequestOptions | CredentialCreationOptions): boolean {
        // Conditional mediation shows the browser's own autofill UI — leave it alone.
        if ((options as CredentialRequestOptions).mediation === 'conditional') return false;
        ...
        if (!(options as any).publicKey) return false;
        return true;
    }

    function userGesture(): boolean {
    ...
    function detachAbort(call: PendingCall) {
        if (call.signal && call.abortHandler) {
            call.signal.removeEventListener('abort', call.abortHandler);
        }
    }

    function watchAbort(signal: AbortSignal | null | undefined, onAbort: () => void): void {
        if (!signal) return;
        signal.addEventListener('abort', onAbort, { once: true });
    }

    /** Ask the content script to run the UI + bridge round-trip. */
    function askContent(type: string, payload: any, signal?: AbortSignal | null): Promise<any> {
        return new Promise((resolve, reject) => {
            const requestId = newRequestId();
            const call: PendingCall = { resolve, reject, signal };
            ...
            watchAbort(signal, () => {
                ...
                window.postMessage(
                    { source: MESSAGE_SOURCE, type: 'PERSONA_PASSKEY_CANCEL', requestId },
                    location.origin
                );
    ...
    function makeCreationCredential(payload: any): PublicKeyCredential {
    ...
    function makeAssertionCredential(payload: any, clientDataJSON: Uint8Array): PublicKeyCredential {
        const rawId = b64urlToBytes(payload.credential_id_b64);
    ...
    navigator.credentials.create = async (options?: CredentialCreationOptions): Promise<Credential> => {
        try {
            if (!options || !shouldIntercept(options)) {
                return (await originalCreate(options)) as Credential;
            }
            ...
            if (!payload) return originalCreate(options) as Promise<Credential>;
            return makeCreationCredential(payload);
        } catch (error) {
            if ((error as DOMException)?.name === 'AbortError') throw error;
            // Persona unavailable — native flow.
            return originalCreate(options) as Promise<Credential>;
        }
    };
    ...
    navigator.credentials.get = async (options?: CredentialRequestOptions): Promise<Credential> => {
        try {
            ...
            if (!payload) return originalGet(options) as Promise<Credential>;
            return makeAssertionCredential(payload, clientData);
        } catch (error) {
            if ((error as DOMException)?.name === 'AbortError') throw error;
            return originalGet(options) as Promise<Credential>;
        }
    };

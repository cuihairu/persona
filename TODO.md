...
- [x] End-to-end encrypted sync (key envelopes, conflict resolution)（2026-09-24
  ...
  - [x] 阶段 2 核心（2026-09-22，批 1–4）：device envelope（DR-1，X25519
        ...
        oplog/push 队列/游标/Lamport 时钟）；服务端密文中继 `/api/v1/sync/*`
        （0004 迁移：设备登记/信封保管/oplog push 幂等/pull 游标分页——非空
        页恒返回游标）；客户端引擎（push/pull 周期 + Lamport 时钟 + travel
        闸 push/pull 双停）；服务端只见密文、重放幂等、乱序容错、换主密码
...
- [x] Reproducible builds（2026-09-24 基线）：deb 产物**同机逐字节可复现**——双构建实测定位不可复现性全部集中在 deb 打包层 tar mtime/属主（前端 dist 与二进制两次构建已逐字节一致，gzip 头 bundler 已置零）；`scripts/normalize-deb.sh` 后处理归一（ar -rD + tar 排序/mtime/属主 + gzip -n），两次 raw 不同的 deb 规范化后同为 `29a0c849…`，`dpkg-deb --info` 结构校验过。工具链已固定（2026-09-24：根仓 `rust-toolchain.toml` 钉 1.97.0 双 workspace 覆盖 + CI 五处 `dtolnay/rust-toolchain@1.97.0`，升级两处一起改）；跨机路径缓解已入仓（`scripts/build-repro.sh`：`--remap-path-prefix=$HOME=/repro-home` + SOURCE_DATE_EPOCH 锚定 HEAD + 归一，opt-in 不影响日常构建）。剩余差距：AppImage/rpm/dmg 归一化未测、真跨机需钉构建容器。方法与数据：`docs/REPRODUCIBLE_BUILDS.md`。**后续（2026-09-24）**：CI 发布面容器浮动已收口——`docker/Dockerfile.server` 两 FROM 按 digest 钉死（系统层冻结；cargo 版本由 rust-toolchain.toml 独立钉 1.97.0 解耦），digest 升级命令入文档；跨机复验所需 deb 构建容器（Dockerfile.repro + 脚本容器模式）仍开放→**同日收口容器半边**：`docker/Dockerfile.repro`（bookworm-slim digest 钉死 + tauri 依赖 + node 24 COPY + corepack pnpm 10.22.0 + rustup 1.97.0 + git 必装——slim 无 git 会让 SOURCE_DATE_EPOCH 锚定静默退化 0，R1 实测暴露后修复）+ `build-repro.sh --docker`（独立 CARGO_TARGET_DIR 防异构缓存污染；CI=true 免 TTY；podman/docker 双引擎）。**容器内同机复现实测通过**：R2/R3 两次独立全量构建（cargo 缓存每次清零）归一产物逐字节同哈希 `000a5265…`（三轮数据见 REPRODUCIBLE_BUILDS「容器内实测」）；剩余半步 = 第二台机器跑 `--docker` 比对同哈希（跨机复验）
